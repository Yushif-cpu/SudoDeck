...
import { payloadsData } from '../../../components/payloadsData';
...
const ENRICHED_PAYLOADS: SecurityPayload[] = payloadsData.map(p => ({
  id: String(p.id),
  category: p.category,
  title: `${p.category} Vector #${p.id}`,
  payload: p.value,
  description: `${p.category} input validation and security audit test probe.`,
  source: 'SecLists / PayloadAllTheThings'
}));

// Built-in resilient fallback catalogue for defensive security testing & input validation
const FALLBACK_PAYLOADS: SecurityPayload[] = [
...
// In-memory cache for live SecLists / remote sync
let cache: { data: SecurityPayload[]; lastFetched: number } | null = null;
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (cache && now - cache.lastFetched < CACHE_TTL_MS) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          const dynamicXss: SecurityPayload[] = lines.map((line, idx) => ({
    ...
    return NextResponse.json(
      {
        success: false,
        error: error?.message || 'Failed to retrieve payload catalog',
        payloads: ENRICHED_PAYLOADS
      },
      { status: 500 }
    );
